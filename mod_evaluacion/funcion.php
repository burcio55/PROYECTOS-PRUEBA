<?php
    ...
    if($accion == 1){
        ...
        if($valor > 0){
            ...
            $SQL2 = "UPDATE public.personales_rol SET rol_id='$rol', nusuario_actualizacion='$usuario' WHERE id = '$id'";
            ...
            if($row = pg_query($conn, $SQL2)){
                ...
                echo "Se presentó un error, favor intentar más tarde / ".$SQL;
                die();
            }
        }
    }else if($accion == 3){
        ...
        if($row = pg_query($conn, $SQL)){
            echo "1 / Se guardó con éxito la información";
            die();
        }else{
        ...
        $SQL = "UPDATE evaluacion_desemp.evaluacion SET nestatus = '5', sobservacion_analista = '$oa' WHERE personales_id = '$id'";
        if($row = pg_query($conn, $SQL)){
            echo "1 / Se guardó con éxito la información";
            die();
        }else{
            echo "0 / Se presentó un error, favor intentar más tarde / ";
            die();
        }
    }else if($accion == 5){
        $id = $_REQUEST['id'];

        $SQL = "UPDATE evaluacion_desemp.evaluacion SET nestatus = '3', sdesacuerdo_evaluado = '1' WHERE id = '$id'";
        if($row = pg_query($conn, $SQL)){
            echo "1 / Se guardó con éxito la información ";
            die();
        }else{
        ...
        $SQL = " UPDATE evaluacion_desemp.evaluacion SET nestatus = '4', sobservaciones = '$text', sdesacuerdo_evaluado = '0' WHERE id = '$id' "; 
            if ($row = pg_query($conn, $SQL)) {
                echo "1 / Se guardó con éxito la información";
                die();
            } else {
