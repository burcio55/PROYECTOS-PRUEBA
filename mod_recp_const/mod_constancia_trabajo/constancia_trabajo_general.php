...
if(isset($_SESSION['id_usuario'])){
						$SQL="SELECT
								...
								WHERE personal.cedula='".$_SESSION['id_usuario']."' AND trabajador.estatus='A' AND concepto.cod_concepto < '5000' And frecuenciapago.cod_frecuencia_pago <='4' AND concepto.cod_concepto <> '0318'  and concepto.cod_concepto <> '0312' and concepto.cod_concepto <> '4009' and concepto.cod_concepto <> '4010' and concepto.cod_concepto <> '4300' and concepto.cod_concepto <> '4301'";	
	...
	echo '<script>alert("Usted sólo puede solicitar tres (03) Constancias de Trabajo Simple con Sueldo en el mes.");</script>';
		 ...
		 $ACUM_CONCEP = " ".$rs1->fields['nombre_concep']." ";
		   ...
		   if($rs1->fields['cod_frecuencia_pago']==3){
			...
			$BS = ' BOLIVARES';// creando variable  bs para bolivares
			$CN = ' CENTIMOS';// creando variable cn para centimos
			$NUM = round($total_asigna, 1);
			$formatterES = new NumberFormatter("es-ES", NumberFormatter::SPELLOUT);
			$izquierda = intval(floor($NUM));
			$derecha = intval(($NUM - floor($NUM)) * 100);
			$letras = $formatterES->format($izquierda) . " Bolivares con " . $formatterES->format($derecha) . " Centimos";
			//$letras = $formatterES->format($NUM);
		 //$totalasigna="<b>".strtoupper(num2letras($pe).$BS.num2letras($PD).$CN.' (Bs.S. '.$NUM.')')."."."</b>";	
		 $totalasigna="<b>".strtoupper($letras.' (Bs.S. '.$NUM.')')."."."</b>";		 
...
<?php }else{?>
   ...
   <tr>
     <th colspan="4"class="titulo" align="center"> Usted alcanzó el máximo de solicitudes de Constancia de trabajo Simple con Sueldo por mes.</th>
   </tr>
