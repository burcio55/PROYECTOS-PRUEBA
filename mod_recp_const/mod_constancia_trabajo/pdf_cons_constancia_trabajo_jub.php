...
$_SESSION['cedula']=$aDefaultForm['txt_cedula'];
$aTabla					= $_SESSION['jubilados'];
$jubilacion 			= number_format( $aTabla['jubilacion'], 2, ',', '.');
$bono_subsistemcia 		= number_format( $aTabla['bono_subsistemcia'], 2, ',', '.');
$pension 				= number_format( $aTabla['pension'], 2, ',', '.');
$jubilacion_tss 		= number_format( $aTabla['jubilacion_tss'], 2, ',', '.');
$bono_subsistemcia 		= number_format( $aTabla['bono_subsistemcia'], 2, ',', '.');
$total					= number_format( $aTabla['total'], 2, ',', '.');
//var_dump($jubilacion);
//die($jubilacion);
$_SESSION['recibo_act'];
$cedula							= number_format($_SESSION['recibo_act']['txt_cedula'], 0, '', '.');
...
$jubilado						= $_SESSION['jubilado'];
$tipo_asignacion				= $_SESSION['tipo_asignacion'];
//$fecha_egreso					= strftime('%d/%m/%Y',strtotime($_SESSION['recibo_act']['txt_fecha_egreso']));
...
//	$can_dias_days= 30;  
	...
	//= date('Y-m-d', strtotime($fecha_hoy) + intval($can_dias_days*24*60*60)); 
	 ...
	 $sql="SELECT personales_cedula, scodigo_constancia
	       ...
	       where personales_cedula='".$_SESSION['id_usuario']."' and tipo_constancias_id=3 and nenabled=1";//COLOCAR VARIABLE OOOOJOOOOOOOOOOOOOOOOOOOOOOOOOOOO
	$rs1 = $conn->Execute($sql);

 $rs1->RecordCount();
	if ($rs1->fields['scodigo_constancia'] == $codigo_const) generar_codigo($conn);
	if ($rs1->RecordCount()<=10){//hasta 3 insert puede hacer al mes 

 $sql2 = "INSERT INTO recibos_pagos_constancias.constancias_trabajo(personales_cedula, tipo_constancias_id, scodigo_constancia, 
            fecha_solicitud_const, fecha_caducidad_const, dfecha_creacion, nusuario_creacion)
			VALUES ('".$_SESSION['id_usuario']."', 3,'".$codigo_const."', now(), '".$fec_caducidad."', now(),'".$_SESSION['id_usuario']."')";
			$conn->Execute($sql2);
		}
...
<tr>
		<td align="justify" style="font-size:100%">Quien suscribe, la <b>Directora General</b> de la <b>Oficina de Gesti&oacute;n Humana del Ministerio del Poder Popular para el Proceso Social de Trabajo</b>, hace constar por medio de la presente que $ciudadano <b>$ape, $nom</b>, titular de la c&eacute;dula de identidad Nro. <b>$nacionalidad.-$cedula</b>, en su condici&oacute;n de $descripcion_cargo, de &eacute;ste Organismo a partir del <b>$fecha_ingreso</b> devengando:</td>
</tr>
</table>
...
<table border="0" cellpadding="2" cellspacing="0">
		<tr>
			<td width="10%" > </td>
			<td width="55%" bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td width="8%" align="center" bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td width="17%" align="center" bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td width="10%" > </td>
		</tr>
		
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Bono de Subsistencia</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">Bs.</td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$bono_subsistemcia</td>
			<td> </td>
		</tr>
		
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Jubilación</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">Bs.</td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$jubilacion</td>
			<td> </td>
		</tr>
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Jubilación Tesorería de Seguridad Social</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">Bs.</td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$jubilacion_tss</td>
			<td> </td>
		</tr>
				<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Pensión</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">Bs.</td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$pension</td>
			<td> </td>
		</tr>
		...
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Total</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">Bs.</td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$total</td>
			<td> </td>
		</tr>
...
/* Pensionado Con Bono de Subsistencia */
$tblPen='';
$tblPen .= <<<EOD
<div align="center" >
<table border="0" cellpadding="2" cellspacing="0">
...
/* Pensionado Sin Bono de Subsistencia */
$tblPenSinSub='';
$tblPenSinSub .= <<<EOD
<div align="center" >
<table border="0" cellpadding="2" cellspacing="0">
...
// NON-BREAKING TABLE (nobr="true")
$pdf->writeHTML($tbl, true, false, false, false, '');
if($jubilacion<>'0,00' and $jubilacion_tss<>'0,00')
{
	$pdf->writeHTML($tblJubTss, true, false, false, false, '');
	//$pdf->writeHTML($tbl3, true, false, false, false, '');
}
elseif($jubilacion<>'0,00' and $jubilacion_tss=='0,00')
{
	$pdf->writeHTML($tblJubSinTss, true, false, false, false, '');
	//$pdf->writeHTML($tbl3, true, false, false, false, '');	
}
elseif($pension<>'0,00' and $bono_subsistemcia<>'0,00')
{
...
elseif($pension<>'0,00' and $bono_subsistemcia=='0,00')
{
	$pdf->writeHTML($tblPenSinSub, true, false, false, false, '');
	//$pdf->writeHTML($tbl3, true, false, false, false, '');	
}
...
$pdf->Output('constancia_trabajo_penjub'.$_SESSION['id_usuario'].'_'.date('d/m/y').'.pdf', 'D');
