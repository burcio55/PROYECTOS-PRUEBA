...
/*var_dump($_SESSION['ciudadano']);
var_dump($_SESSION['adscrito']);
var_dump( $_SESSION['figura']);
var_dump($_SESSION['recibo_act']);
var_dump($_SESSION['fecha_solicitud_const']);
 var_dump($_SESSION['total_asigna']);
var_dump($_SESSION['totalasigna']);
var_dump($_SESSION['monto_tickets']);*/
...
//	$can_dias_days= 30;  
 ...
 $sql2 = "INSERT INTO recibos_pagos_constancias.constancias_trabajo(
						personales_cedula, 
						tipo_constancias_id, 
						scodigo_constancia,
						...
						nusuario_creacion, 
						nmonto, 
						tickets_alimentacion_nmonto)
			VALUES ('".$_SESSION['recibo_act']['txt_cedula']."',
					10,
					'".$codigo_const."', 
					now(), 
					'".$fec_caducidad."', 
					now(),
					'".$_SESSION['id_usuario']."',
					".$_SESSION['total_asigna'].",
					'".$_SESSION['monto_tickets']."')";
			$conn->Execute($sql2);
		}
...
<tr  >
	<td colspan="4" align="justify" style="font-size:100%">Adicionalmente, percibe por concepto de Cestaticket Socialista, <b>$tickets</b> mensual.</td>
</tr>
</table>
EOD;
...
$pdf->Output('constancia_trabajo_csuel'.$_SESSION['recibo_act']['txt_cedula'].'_'.date('d/m/y').'.pdf', 'D');
