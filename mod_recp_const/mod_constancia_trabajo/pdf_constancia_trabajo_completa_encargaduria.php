...
$pdf->SetTitle('Constancia de Trabajo Completa Encargaduria');
$pdf->SetSubject('Constancia de Trabajo Completa Encargaduria');
...
$nom 							= htmlentities(trim($_SESSION['recibo_act']['txt_nombre1'].' '.$_SESSION['recibo_act']['txt_nombre2']),ENT_QUOTES);
$ape			 	  			= htmlentities($_SESSION['recibo_act']['txt_apellido1'].' '.$_SESSION['recibo_act']['txt_apellido2'],ENT_QUOTES);
...
$encargaduria					= $_SESSION['recibo_act']['txt_encargaduria'];
$total_asigna					= $_SESSION['montos']['total_asigna'];
$bono_vacacional				= $_SESSION['montos']['bono_vacacional'];
$bono_fin_ano					= $_SESSION['montos']['bono_fin_ano'];
$ayuda_econ						= $_SESSION['montos']['ayuda_econ'];
...
$designado						= $_SESSION['designado'];
$encargado						= $_SESSION['encargado'];

//var_dump ($_SESSION['montos']);
function generar_codigo($conn,$total_asigna){
...
//	$can_dias_days= 30;  
	...
	//= date('Y-m-d', strtotime($fecha_hoy) + intval($can_dias_days*24*60*60)); 
	 ...
	 $sql="SELECT personales_cedula, scodigo_constancia
	       ...
	       where personales_cedula='".$_SESSION['id_usuario']."' and tipo_constancias_id=7 and nenabled=1";//COLOCAR VARIABLE OOOOJOOOOOOOOOOOOOOOOOOOOOOOOOOOO
	$rs1 = $conn->Execute($sql);


 $rs1->RecordCount();
	if ($rs1->fields['scodigo_constancia'] == $codigo_const) generar_codigo($conn,$total_asigna);
 ...
 $sql2 = "INSERT INTO recibos_pagos_constancias.constancias_trabajo(personales_cedula, tipo_constancias_id, scodigo_constancia, 
			...
			VALUES ('".$_SESSION['id_usuario']."', 7,'".$codigo_const."', now(), '".$fec_caducidad."', now(),'".$_SESSION['id_usuario']."',
			".$total_asigna.",".$_SESSION['monto_tickets'].")";
			$conn->Execute($sql2);
		}
...
$codigo_generado 	          = generar_codigo($conn,$total_asigna);
$_SESSION['txt_codigo_cod']   =$codigo_generado;
$fec_caducidad 		          = strftime('%d/%m/%Y',strtotime($GLOBALS['fec_caducidad']));
//$URL = 'http://10.46.1.91/minpptrass/mod_recp_const/mod_consulta_externas/consulta_constancia_certificada.php';
...
<tr>
		<td align="justify" style="font-size:100%">Quien suscribe, la <b>Directora General</b> de la <b>Oficina de Gesti&oacute;n Humana</b>, hace constar por medio de la presente que $ciudadano <b>$ape, $nom</b>, titular de la c&eacute;dula de identidad Nro. <b>$nacionalidad.-$cedula</b>, presta sus servicios en &eacute;ste Organismo, desde el <b>$fecha_ingreso</b>, desempe&ntilde;ando el cargo de <b>$descripcion_cargo</b>, $adscrito <b> $descripcion_ubicacion_adm</b>, quien actualmente se encuentra como <b>$encargaduria</b>, devengando un:</td>
</tr>
</table>
EOD;

$total_asigna = number_format($total_asigna, 2, ',', '.');
...
<table border="0" cellpadding="2" cellspacing="0">
		<tr>
			<td width="15%" > </td>
			<td width="45%" bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td width="8%" align="center" bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td width="21%" align="center" bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td width="11%" > </td>
		</tr>
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Ingreso Mensual</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">BsS. </td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$total_asigna</td>
			<td> </td>
		</tr>
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Bono Vacacional</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">BsS. </td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$bono_vacacional</td>
			<td> </td>
		</tr>
		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Bonificaci&oacute;n de Fin de A&ntilde;o</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">BsS. </td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$bono_fin_ano</td>
			<td> </td>
		</tr>

		<tr>
			<td> </td>
			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Cestaticket Socialista</td>
			<td align="center" bgcolor="#F0F0F0" color="#1060C8">BsS. </td>
			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$monto_tickets</td>
			<td> </td>
		</tr>
		<tr>
			<td> </td>
			<td bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td bgcolor="#D0E0F4" color="#1060C8"> </td>
			<td> </td>
		</tr>
...
<tr>
	 <td align="center"><img src="../img_firmas/firmaGLADYS.jpg" height="130"></td>
</tr>
<tr>
     <td align="center" style="font-size:100%"><b>GLADYS OMAIRA SU&Aacute;REZ RAM&Iacute;REZ</b></td>
</tr>
<tr>
     <td align="center" style="font-size:73%"><b>DIRECTORA GENERAL DE LA OFICINA DE GESTI&Oacute;N HUMANA</b></td>
</tr>
<tr>
     <td align="center" style="font-size:78%">Seg&uacute;n Resoluci&oacute;n N&deg; 709 de fecha 20/12/2018</td>
</tr>
<tr>
     <td align="center" style="font-size:78%">Gaceta Oficial N&deg; 41.552 de fecha 26/12/2018</td>
</tr>
<tr>
...
<tr>
	<td align="center" style="font-size:62%">Esta constancia ha sido impresa electr&oacute;nicamente, los datos reflejados estan sujetos a confirmaci&oacute;n a trav&eacute;s de: <a href="$URL">www.mpppst.gob.ve</a>, Solicitudes en Línea, Consultas, Certificaci&oacute;n de Constancia de Trabajo, Valido hasta el <b>$fec_caducidad</b></td>
</tr>
<tr>
...
<tr>
  <td align="center" style="font-size:65%">telef&oacute;nicos:(0212)-619 6749; (0212)-408 4366, 4374, 4375,4380.</td>
</tr>
</table>
EOD;
...
//		<tr>
//			<td> </td>
//			<td align="left" bgcolor="#F0F0F0" color="#1060C8">Contribuci&oacute;n Econ&oacute;mica Familiar *</td>
//			<td align="center" bgcolor="#F0F0F0" color="#1060C8">Bs.</td>
//			<td align="right" bgcolor="#F0F0F0" color="#1060C8">$ayuda_econ</td>
//			<td> </td>
//		</tr>
...
$pdf->Output('constancia_trabajo_comp_encarg'.$_SESSION['id_usuario'].'_'.date('d/m/y').'.pdf', 'D');
