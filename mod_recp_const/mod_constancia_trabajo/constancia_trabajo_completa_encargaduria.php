	...
	if (count($GLOBALS['aDefaultForm']) == 0){
			$aDefaultForm = &$GLOBALS['aDefaultForm'];
					...
					$SQL="SELECT personal.id_personal,
							    ...
							    encargaduria.cargo as encargaduria
								FROM trabajador
								INNER JOIN personal on personal.id_personal = trabajador.id_personal
								inner join encargaduria on encargaduria.id_personal = trabajador.id_personal
								...
								WHERE personal.cedula='".$_SESSION['id_usuario']."' and estatus='A' and trabajador.id_tipo_personal<>'29'";
...
if(isset($_SESSION['id_usuario'])){
						$SQL="SELECT
								...
								WHERE personal.cedula='".$_SESSION['id_usuario']."' AND concepto.cod_concepto < '5000' And frecuenciapago.cod_frecuencia_pago <='4' and trabajador.estatus='A' and trabajador.id_tipo_personal<>'29' and concepto.id_concepto<>'1981'";	
...
<form name="frm_constancia_trabajo_personal_activo" id="frm_constancia_trabajo_personal_activo" method="post" action="<?= $_SERVER['PHP_SELF'] ?>" > 
    ...
    <table class="formulario" width="58%" align="center">
        ...
        <tr>
         <th colspan="2" class="titulo" align="center">CONSTANCIA DE TRABAJO COMPLETA ENCARGADURIA</th>    
        </tr>
        
        <tr>
            <td colspan="2" width="50%" height="200%">
            <p align="justify"><font color="#666666">Quien suscribe, la <b>Directora General</b> de la <b>Oficina de Gesti&oacute;n Humana</b>, hace constar  por medio de la presente que 
            <?php echo $_SESSION['ciudadano'];?> <b><? print trim($aDefaultForm['txt_apellido1']." ".$aDefaultForm['txt_apellido2']).",  ".trim($aDefaultForm['txt_nombre1']." ".$aDefaultForm['txt_nombre2']);?></b>,  titular de la c&eacute;dula de identidad Nro. <b><?php echo $aDefaultForm['txt_nacionalidad']; echo '.-'.number_format($_SESSION['id_usuario'], 0, '', '.');?></b>, presta sus servicios en &eacute;ste Organismo, desde el  <b><?= strftime("%d/%m/%Y", strtotime($aDefaultForm['txt_fecha_ingreso']));?></b>, desempe&ntilde;ando el cargo de <b><?= $aDefaultForm['txt_descripcion_cargo'];?></b>, <? echo $_SESSION['adscrito'];?><b><?= $aDefaultForm['txt_nombre_dep'];?></b>, quien actualmente se encuentra  como <b><?= $aDefaultForm['txt_encargaduria'];?></b>, devengando un: <? 
            $total_asigna = 0;
...
if ($_SESSION['ncontador']<3){ ?>
  <tr>
     <td colspan="5" align="center">
         <button type="submit"  class="button_personal btn_imprimir"  formaction="pdf_constancia_trabajo_completa_encargaduria.php" formtarget="_blank" title="Haga Click para Imprimir la Constancia de Trabajo">Imprimir</button></td>
   </tr>
