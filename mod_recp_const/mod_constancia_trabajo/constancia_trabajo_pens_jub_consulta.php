...
function showForm($conn,$aDefaultForm){ global $conn;
global $conn2;
if(isset($_SESSION['id_usuario'])){
			   $SQL="SELECT concepto.cod_concepto as codigoconcep,
					...
					WHERE personal.cedula='".$_SESSION['id_usuario']."' AND concepto.cod_concepto in ('0010','0026','0011','3415') order by concepto.cod_concepto desc ";		
					$rs1=$conn2->Execute($SQL);
				}
	$sql="SELECT personales_cedula, scodigo_constancia
           FROM recibos_pagos_constancias.constancias_trabajo
	       where personales_cedula='".$_SESSION['id_usuario']."' and tipo_constancias_id=8 and nenabled=1";//COLOCAR VARIABLE OOOOJOOOOOOOOOOOOOOOOOOOOOOOOOOOO  and dfecha_creacion>'2018-06-01'
...
<form name="frm_constancia_trabajo_pens_jub_consulta" id="frm_constancia_trabajo_pens_jub_consulta" method="post" action="<?= $_SERVER['PHP_SELF'] ?>" >
...
<table width="100%" border="0" align="center" class="formulario" cellpadding="3" cellspacing="0">
  ...
  <tr>
    <th colspan="4"  class="titulo" align="center">CONSTANCIA DE  JUBILADO/PENSIONADO</th>
  </tr>
  
        <tr>
        <th colspan="4"  class="sub_titulo"><div align="left">CONSTANCIAS DE TRABAJO--> Jubilado/Pensionado</div></th>        
      </tr>
...
<table class="formulario" width="58%" align="center">
 	   ...
 	   <tr>
      <td width="50%" height="200%"><font color="#666666"><p align="justify">Quien suscribe, la <b>Directora General</b> de la <b>Oficina de Gesti&oacute;n Humana del Ministerio del Poder Popular para el Proceso Social de Trabajo</b>, hace constar  por medio de la presente que <?php echo $_SESSION['ciudadano'];?> <b><?php print trim($aDefaultForm['txt_apellido1']." ".$aDefaultForm['txt_apellido2']).",  ".trim($aDefaultForm['txt_nombre1']." ".$aDefaultForm['txt_nombre2']);?></b>, titular de la c&eacute;dula de identidad Nro. <b><?php echo $aDefaultForm['txt_nacionalidad']; echo '-'.number_format($_SESSION['id_usuario'], 0, '', '.');?></b>, en su condici&oacute;n de <b><?php echo $_SESSION['jubilado'];?></b>, de &eacute;ste Organismo a partir del <b><?= strftime("%d/%m/%Y", strtotime($aDefaultForm['txt_fecha_ingreso']));?></b>, devengando un: <? 
					$total_asigna = 0;
...
if ($_SESSION['ncontador']<3){ ?>
  <tr>
     <td colspan="5" align="center">
         <button type="submit"  class="button_personal btn_imprimir"  formaction="pdf_constancia_trabajo_pens_jub_consulta.php" formtarget="_blank" title="Haga Click para Imprimir la Constancia de Trabajo">Imprimir</button></td>
   </tr>
