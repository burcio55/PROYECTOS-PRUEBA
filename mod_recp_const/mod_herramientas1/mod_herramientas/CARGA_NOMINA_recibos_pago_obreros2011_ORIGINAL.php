...
if(isset($_POST['semana']) <> 0)
{

	  $sql= "select trabajador.cedula, trabajador.cod_tipo_personal, trabajador.estatus,  
	         trabajador.forma_pago,trabajador.cuenta_nomina,personal.primer_apellido,     
			 personal.primer_nombre,trabajador.sueldo_basico, dependencia.nombre,  
			 cargo.descripcion_cargo,historicosemana.mes, historicosemana.semana_quincena,
			 historicosemana.id_concepto_tipo_personal,concepto.cod_concepto,
			 concepto.descripcion,historicosemana.monto_asigna,
			 historicosemana.monto_deduce,tipopersonal.nombre as tipopersonal,
			 trabajador.codigo_nomina,historicosemana.numero_nomina,	
                         trabajador.cod_tipo_personal as cod_tipo_trabajador,
...
for ($j=0; $j < $filas; $j++) {
   ...
   $primer_apellido= pg_result($resultado_set, $j, 5);
   $primer_nombre= pg_result($resultado_set, $j, 6);
   $sueldo_basico= pg_result($resultado_set, $j, 7);
   $nombre= pg_result($resultado_set, $j, 8);
   $descripcion_cargo= pg_result($resultado_set, $j, 9);
   $mes= pg_result($resultado_set, $j, 10);
   $semana_quincena= pg_result($resultado_set, $j, 11);
   $id_concepto_tipo_personal= pg_result($resultado_set, $j, 12);
   $cod_concepto= pg_result($resultado_set, $j, 13);
   $descripcion= pg_result($resultado_set, $j, 14);
   $monto_asigna= pg_result($resultado_set, $j, 15);
   $monto_deduce= pg_result($resultado_set, $j, 16);
   $tipopersonal= pg_result($resultado_set, $j, 17);
   $cod_nomina=pg_result($resultado_set, $j, 18);
   $nro_nomina=pg_result($resultado_set, $j, 19);
...
$conexion1 = pg_connect("user=jpalacios dbname=recibos_pagos host=10.46.1.23 password='12345'");
  $sql1="INSERT INTO   recibos_pagos(cedula,cod_tipo_personal,estatus,forma_pago,cuenta_nomina,primer_apellido, primer_nombre,sueldo_basico, nombre,descripcion_cargo,mes, semana_quincena, id_concepto_tipo_personal, cod_concepto,descripcion, monto_asigna, monto_deduce,tipo_personal,anio,fecha_carga,cod_nom,nro_nomina,cod_tipo_trabajador,cod_ubicacion_adm,id_cargo)
...
<form name="form" method="post" action="recibos_pago_obreros2011.php" >
<script>
    function send(saction){
       ...
       <tr>
           <td colspan="2" class="titular" align="center">Generar Nomina para recibos de Obreros </td>
       </tr>
       <tr>
