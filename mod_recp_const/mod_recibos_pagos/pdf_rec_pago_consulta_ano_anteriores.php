...
$pdf->SetTitle('Recibo de Pago de Quincenal Años Anteriores');
$pdf->SetSubject('Recibo de Pago de Quincenal Años Anteriores');
...
function cambiarMayusculas($cadena){
	...
	return $cadena;
						...
						$quincena 										= str_replace('QUINCENA DEL ','',$recibos['quincena']);
						$mes	 										= $recibos['mes'];
    					$anio	 										= $recibos['anio'];
						$fecha = date('d/m/Y');
		...
		if ($cod_tipo_trabajador!='05')	  $query = $quincena;
		     ...
		     $sql3="SELECT recibos_pagos_constancias.conceptos.scodigo as cod_concepto, 
					...
					and historico_recibo_pago.nsemana_quincena='".$_GET['sq']."'
...
$pdf->Output('recibo_pagqaant'.$_SESSION['id_usuario'].'_'.date('d/m/y').'.pdf', 'D');
