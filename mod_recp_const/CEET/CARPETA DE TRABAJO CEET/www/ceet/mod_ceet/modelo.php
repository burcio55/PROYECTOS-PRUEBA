...
if ($_POST["elegido"]=='239') { $param= " and pais_id='239'"; }
	else{ $param= " and pais_id='-1'"; }
...
if ($_POST["combo"]=='Curso'){
	$sSQL="SELECT id, nombre, status  FROM public.curso where status='A' and categoria_curso_id = '".$_POST["elegido"]."' ORDER BY id";	
	//echo $sSQL;
	$rs = &$conn->Execute($sSQL); 
	 $options.= "<option value=''>Seleccione</option>"; 
	while (!$rs->EOF ){
