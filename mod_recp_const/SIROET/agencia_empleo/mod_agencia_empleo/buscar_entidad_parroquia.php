...
$municipio=str_replace("'","",$_REQUEST['municipio']);
$municipio=str_replace(",","",$municipio);
$municipio=str_replace("-","",$municipio);
$municipio=htmlentities((trim($municipio)));
}

if(isset($_REQUEST['parroquia'])){
$parroquia=str_replace("'","",$_REQUEST['parroquia']);
$parroquia=str_replace(",","",$parroquia);
$parroquia=str_replace("-","",$parroquia);
$parroquia=htmlentities((trim($parroquia)));
}

//CONTADOR TOTAL DE REGISTROS POR CADA PARROQUIA
$SQLL="select empresa_instituto.* 
	  		from empresa_instituto
	  		left JOIN parroquia ON empresa_instituto.municipio_id=parroquia.id
			where empresa_instituto.status='A'  and empresa_instituto.estado_id = '".$_POST['estado']."' and empresa_instituto.municipio_id = '".$_POST['municipio']."' and empresa_instituto.parroquia_id = '".$_POST['parroquia']."' order by  empresa_instituto.id";
...
$SQL="select distinct empresa_instituto.*, empresa_instituto.id as empresa_id, 
			...
			left JOIN municipio ON empresa_instituto.estado_id=municipio.estado_id
			left JOIN parroquia ON empresa_instituto.municipio_id=parroquia.municipio_id
...
<script type="text/javascript">
				// Create the chart
				Highcharts.chart('reporte_por_parroquia', {
				    chart: {
				        type: 'column'
				    },
				    lang: {
