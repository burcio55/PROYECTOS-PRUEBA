...
function doAction($conn){
	...
	if (isset($_SESSION['id_empresa'])){
		$_SESSION['registro']='1';
		?><script>document.location='?menu=41'</script><? 
		} 
		else{
	   ...
	   if (!ereg("^[0-9]{8}$", $_POST['Rif'])){ 
		    ...
		    $_POST['rif']=$_POST['cbRif'].$_POST['Rif'].$_POST['cbRif1'];
			 $SQL = "select *  from empresa_instituto 
					...
					$rs = $conn->Execute($SQL);							        
					if ($rs->RecordCount()>0){
					   $capacitacion=$rs->fields['prog_cap'];
					   if ($capacitacion=='1'){
			 ...
			 $SQL1="select ocupacion.nombre as ocupacion, 
				...
				if ($rs1->RecordCount()>0){	
			       ...
			       $_SESSION['registro']='1';
				   ?><script>if (confirm("- Esta entidad de trabajo no tiene Oportunidades de Formación asociadas. Desea agregar una?"))
					document.location="?menu=41";
					else document.location="?menu=40"
					</script><?
				   }
				   }  
		    	else{
					$GLOBALS['aPageErrors'][]= "- Esta entidad de trabajo no imparte programa de formación.";
					$bValidateSuccess=false;
					}	  
			   	  
				}
					else{
					$GLOBALS['aPageErrors'][]= "- Esta entidad de trabajo no se encuentra registrada.";
					$bValidateSuccess=false;
					}
		 	}
			LoadData($conn,true);
...
function send(saction){
 ...
 <table width="100%" border="0" align="center" class="formulario" cellpadding="0" cellspacing="0">   
    ...
    <tr>
    		<th colspan="2" class="titulo">REGISTRO DE OPORTUNIDADES FORMATIVAS</th>
    </tr>
  <!--  <tr>
 ...
 <td><div align="center">
  ...
  <table  border="0" align="center"  class="listado formulario" id="tblDetalle" style="width:99%; ">
    ...
    <tr align="center">
      ...
      <th width="15%" class="labelListColumn">Categoria </th>
      ...
      <th width="7%" class="labelListColumn" >Cupos disponibles </th>
      <th width="6%" class="labelListColumn"> Inicia </th>
      <th width="6%" class="labelListColumn"> Culmina </th>
      <th width="6%" class="labelListColumn"> Horario </th>
      <th width="7%" class="labelListColumn"> Jornada </th>
      <th width="7%" class="labelListColumn"> Colectivos </th>
      <th width="5%" class="labelListColumn"> Status </th>
      <th width="3%" class="labelListColumn"></th>
    </tr>
    <?
    ...
    <tr class="<?=$class_name?>">
      ...
      <td class="texto-normal"><a target="new" href="?formato=6&id_po=<?=$aTabla[$i]['id']?>"><img src="imagenes/eye.png" width="12" height="12" border="0" title="Ver Oferta"/></a><a href="?menu=41&id_po=<?=$aTabla[$i]['id']?>"><img src="imagenes/b_edit.png" width="12" height="12" border="0" title="Editar" /></a></td>
    </tr>
    <? } ?>
  </table>
   <tr>
  ...
  <tr>
      <td colspan="13"><div align="right"><button type="button" name="Agregar"  id="Agregar" class="button"  onClick="javascript:send('Agregar');">Registrar oportunidad de formaci&oacute;n</button></div></td>
    </tr>
</form>
<?php
}
