...
function doAction($conn){
	if (isset($_POST['action'])){
		switch($_POST['action']){
			...
			case 'Cancelar': 
			?><script> 
				    document.location='?menu=17'</script><?
				/*unset($_POST['id_po']);
				unset($_POST['accionI']);
				unset($_POST['accionC']);
				unset($_POST['bloq1']);
...
//-----------------AGREGAR OTRA HABILIDAD PERSONA-----------------------------------------------------------------		
			...
			case 'Continuar': 
					...
					unset($_POST['id_po']);
					unset($_POST['accionC']); 
					?><script>document.location='?menu=18'</script><?
			break;
	        }
		}		
		else{	
...
<form name="frm_conocimientos" method="post" action="" >
<script>
...
<table width="100%" border="0" align="center" class="formulario" cellpadding="0" cellspacing="0">
	 ...
	 <tr>
          ...
          <tr>
          <th colspan="3" class="titulo">OTROS CONOCIMIENTOS, HABILIDADES Y DESTREZAS</th>
          </tr>
          <tr>
          <th colspan="3" class="sub_titulo" align="left">Dominio de computación: </th>
          </tr>
          <tr>
          <td width="40%"><div align="right">Herramienta de Computaci&oacute;n:</div></td>
          <td width="60%" colspan="2"><span class="links-menu-izq">
          <select name="cbComputacion" class="tablaborde_shadow" id="cbComputacion" <? if($_POST['accionC']=="1"){ ?> disabled <? }?>>
          ...
          <tr>
          <td><div align="right"><span class="links-menu-izq">Agregar otra herramienta de computación: </span></div></td>
          <td colspan="2"><span class="links-menu-izq">
          <input name="otra_herramienta" type="text" class="tablaborde_shadow" id="otra_herramienta" value="<?=$aDefaultForm['otra_herramienta']?>"size="20" maxlength="50" />
          ...
          <td colspan="2"><select name="cbNivel_compu" class="tablaborde_shadow" id="cbNivel_compu">
          <option value="-1" selected="selected">Seleccione</option>
          ...
          <? if($_POST['accionC']=="1"){ ?>
          <button type="button" name="Actualizar"  id="Actualizar" class="button"  onClick="javascript:send('Agrega_compu');">Actualizar</button>
          <? }
          else{ ?>
          <button type="button" name="Agregar"  id="Agregar" class="button"  onClick="javascript:send('Agrega_compu');">Agregar</button>
          </span>
          <? }?>
          <span class="requerido">
          ...
          <tr>
          <td colspan="3">
                <table  border="0"  align="center" class="listado formulario" id="tblDetalle" style="width:80%; ">
                <tr align="center" >
                    <th width="60%" class="labelListColumn">Herramientas de Computación </th>
                    <th width="33%" class="labelListColumn">Nivel</th>
                ...
                <td class="texto-normal"><a href="?menu=17&id_po=<?=$aTabla[$i]['id']?>&accionC=1"><img src="imagenes/pencil_16.png"  border="0" title="Editar" /></a> <a href="?menu=17&id_po=<?=$aTabla[$i]['id']?>&accionC=2"><img src="imagenes/delete_16.png"  border="0" title="Eliminar" /></a> </td>
                </tr>
                <? } ?>
                </table>    
      </td>
      ...
      <tr>
      <th colspan="3" class="sub_titulo" align="left">Otras habilidades y destrezas: </th>
      </tr>
      <tr>
      <tr>
      <td width="40%"><div align="right">Descripci&oacute;n de competencias, habilidades y destrezas para el trabajo:</div></td>
      <td width="60%" colspan="2"><textarea name="Otra_habilidad" cols="60" class="tablaborde_shadow" id="Otra_habilidad"><?=$aDefaultForm['Otra_habilidad']?></textarea>
      </td>
      </tr>
      ...
      <td colspan="3" align="center">
         <button type="button" name="Agregar_Habil"  id="Agregar_Habil" class="button"  onclick="javascript:send('Agregar_Habil')">Agregar</button>
         <button type="button" name="Cancelar"  id="Cancelar_Habil" class="button"  onclick="javascript:send('Cancelar_Habil');">Cancelar</button>
