<?php
$conn->debug = true;
//------------------pais experiencia laboral en el exterior---------------------
function Loadcbpais_donde_laboro($conn){  
	$sHtml_Var = "sHtml_cbpais_donde_laboro";
	if (!isset($GLOBALS[$sHtml_Var])){
		$GLOBALS[$sHtml_Var] = '';
	}
	if ($GLOBALS[$sHtml_Var] == ''){
		$sSQL="SELECT id, nombre, status  FROM public.pais where status='A' and id !='239' ORDER BY cod";		
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
			$GLOBALS[$sHtml_Var] .= "<option value='".$rs->fields['0']."'";
			if ($rs->fields['0'] == $GLOBALS['aDefaultForm']['cbpais_donde_laboro']) {
...
//-----Redes Sociales
function LoadRedes_Sociales($conn){  
	$sHtml_Var = "sHtml_cbred_social";
	if (!isset($GLOBALS[$sHtml_Var])){
		$GLOBALS[$sHtml_Var] = '';
	}

	if ($GLOBALS[$sHtml_Var] == ''){
		$sSQL="SELECT id, nombre, status  FROM public.redes_sociales where status='A'  order by nombre";		
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
			$GLOBALS[$sHtml_Var] .= "<option value='".$rs->fields['0']."'";
			if ($rs->fields['0'] == $GLOBALS['aDefaultForm']['cbred_social']) {
				$GLOBALS[$sHtml_Var].= " selected='selected'";
			}
			$GLOBALS[$sHtml_Var] .= ">".$rs->fields['1']." </option>\n";
			$rs->MoveNext();
...
function LoadJornada_interes($conn){  
	...
	if ($GLOBALS[$sHtml_Var] == ''){
		$sSQL="SELECT id,  nombre, status  FROM public.turno_jornada where status='A' ORDER BY nombre";		
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
			$GLOBALS[$sHtml_Var] .= "<option value='".$rs->fields['0']."'";
...
function LoadCentro_capacitacion($conn){  
	...
	if ($GLOBALS[$sHtml_Var] == ''){
		$sSQL="SELECT id, nombre, status  FROM public.centro_capacitacion where status='A' ORDER BY nombre";		
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
			$GLOBALS[$sHtml_Var] .= "<option value='".$rs->fields['0']."'";
...
function LoadComputacion($conn){  
	...
	if ($GLOBALS[$sHtml_Var] == ''){
		$sSQL="SELECT id, nombre, status  FROM public.computacion where status='A' ORDER BY nombre";		
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
			$GLOBALS[$sHtml_Var] .= "<option value='".$rs->fields['0']."'";
