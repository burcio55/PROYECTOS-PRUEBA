...
<?php if (!isset($login)) { ?>
	...
	<div id="contenedor" align="center">
		...
		<div id="Encabezado">
			...
			<?php
				@$diasemena = date('w');
				if($diasemena == 0){ $dialetra = "Domingo";}
				if($diasemena == 1){ $dialetra = "Lunes";}
				if($diasemena == 2){ $dialetra = "Martes";}
				if($diasemena == 3){ $dialetra = "Miercoles";}
				if($diasemena == 4){ $dialetra = "Jueves";}
				if($diasemena == 5){ $dialetra = "Viernes";}
				if($diasemena == 6){ $dialetra = "Sabado";}
				@$diames = date('j');
				@$mes = date('m');
				if($mes == '01'){ $mesletra = "Enero";}
				if($mes == '02'){ $mesletra = "Febrero";}
				if($mes == '03'){ $mesletra = "Marzo";}
				if($mes == '04'){ $mesletra = "Abril";}
				if($mes == '05'){ $mesletra = "Mayo";}
				if($mes == '06'){ $mesletra = "Junio";}
				if($mes == '07'){ $mesletra = "Julio";}
				if($mes == '08'){ $mesletra = "Agosto";}
				if($mes == '09'){ $mesletra = "Septiembre";}
			...
			<table width="100%" border="0" >
				<tr>
					<th width="11%" class="usuario"><? if( isset($_SESSION['nombre1'])) {?>USUARIO ACTIVO:<? }?></th>
					<td width="69%" class="usuariodescripcion"><?php echo $_SESSION['apellido1']." ".$_SESSION['apellido2']." ".$_SESSION['nombre1']." ".$_SESSION['nombre2']; ?></td>
					<th width="30%" class="fechahora"><div align="right"><?=$dialetra.", ".$diames." de ".$mesletra." del ".$ano; ?></div></th>
				</tr>
				<?php if(isset($_SESSION['sistema'])){ ?>
				<tr>
					<th colspan="3"><strong><div align="center"><?php echo $_SESSION['sistema'];?></div></strong></th>
					<?php 
				...
				<?php }else{ ?>
					<?php if(!isset($registro_personal)){ ?>
						<?php if(!isset($olvido_contrasena)){ ?>
							<tr>
								<th colspan="3"><strong><div align="center">MODULOS</div></strong></th>
							</tr>
						<?php }else{ ?>
							<tr>
								<th colspan="3"><strong><div align="center">OLVIDO CONTRASEÑA</div></strong></th>
							</tr>
						<?php } ?>
					<?php }else{ ?>
						<tr>
							<th colspan="3"><strong><div align="center">DATOS DEL TRABAJADOR</div></strong></th>
						</tr>
					<?php } ?>
