...
class dateObj {
	...
	function date_to_array( $dateTime = '' ) {
		...
		if ( $timePos ) {
			...
			$time = substr( $dateTime, $timePos+strlen( $this->timeSeparator), strlen($dateTime) );
		} else $date = $dateTime;
		### Traitement date
		...
		if ( !$dateIsValid ) {
			trigger_error( '<p>class dateObj::date_to_array($date) --> la date fournie est invalide!<br>Date fournie : '.$dateTime.'</p>', E_USER_ERROR );
		}
	...
	function diff( $date, $retMode = '', $roundMode = 'ceil' ) {
		if ( empty($date) ) {
			trigger_error("class dateObj::diff($date) --> la date à soustraire n'est pas définie.", E_USER_ERROR );
		} else {
