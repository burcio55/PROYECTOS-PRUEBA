...
if ($id_capacitacion != '') {
    ...
    $pdf->Cell(80, 7, $cap, 0, 0, 'L');
    $pdf->Cell(70, 7, $int_cap, 0, 0, 'L');
    ...
    do {
        ...
        $pdf->SetX(8);
        $pdf->Cell(80, 7, $cap2, 0, 0, 'L');
        $pdf->SetX(88); // Ajusta la posición X para el MultiCell
        $y = $pdf->GetY(); // Obtiene la posición Y actual
        $pdf->SetY($y); // Mantiene la posición Y para la celda de duración
        $pdf->SetX(158); // Ajusta la posición X para la celda de duración
        if ($dur2 > 1) {
            $pdf->Cell(0, 7, $dur2 . " HORAS", 0, 0, 'L');
        } else if ($dur2 == 1) {
            $pdf->Cell(0, 7, 'UNA HORA', 0, 0, 'L');
        } else {
            $pdf->Cell(0, 7, 'NO CULMINADO', 0, 0, 'L');
        }
        $pdf->SetY($y); // Restablece la posición Y para el MultiCell
        $pdf->SetX(88); // Ajusta la posición X para el MultiCell
        $pdf->MultiCell(70, 5, $int_cap2, 0, 'L');
...
if ($id_experiencia != '') {
    ...
    do {
        ...
        // Renderizamos el nombre y ajustamos la altura de la celda
        $pdf->SetX(8);
        $pdf->MultiCell(90, 7, $name_tra2, 0, 'L');
        
        // Guardamos la nueva posición Y después de renderizar el nombre
        $newPosY = $pdf->GetY();
    
        // Volvemos a la posición Y inicial para renderizar el cargo en la misma altura
        $pdf->SetY($posY);
        $pdf->SetX(98);
        $pdf->Cell(0, 7, $cargo2, 0, 1, 'L');
    
        // Movemos la posición Y al final de la celda de nombre para continuar con el ingreso y egreso
        $pdf->SetY($newPosY);
        $pdf->SetX(8);
