...
$user=$nacionalidad.$usuario;
$SQL="SELECT * FROM usuarios WHERE cedula = '".$user."' AND clave = '".trim(md5($clave))."' ";
...
$rs=$conn->Execute($SQL);
	...
	if($_SESSION["captcha"]==$captcha){
			if($rs->RecordCount()>0){
				...
				$_SESSION['nacionalidad']=$rs->fields['nacionalidad'];
				$_SESSION['captcha']=$rs->fields['captcha'];
				$_SESSION['nusuario']=$rs->fields['cedula'];
				$_SESSION['nombre']=strtoupper($rs->fields['nombres']);
				$_SESSION['apellido']=strtoupper($rs->fields['apellidos']);
				$_SESSION['id_clave']=trim(md5($clave));
			
				$_SESSION['ntipo']=$rs->fields['tipo_usuario'];
