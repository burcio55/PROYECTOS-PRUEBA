<?php
    ...
    while ($row = pg_fetch_assoc($valor3)) {
        ...
        echo ('<tr><td>'.$i.'</td><td>'.$row['snombre_otros_conocimientos'].'</td><td>'.$row['sdominio_conocimiento'].'</td><td><input type="button" class="btn btn-secondary" style="border-radius: 30px; display: inline-flex;" value="Editar" onclick="modificar_conocimientos('.$row['id'].');"><input type="button" class="btn btn-danger" style="border-radius: 30px;display: inline-flex;" value="Borrar" onclick="eliminar_conocimientos('.$row['id'].');"></td></tr>');
    }
?>