...
if($accion == 1){
    ...
    if ($tipo_beneficio == '-1') {
        echo "Error, el campo \"Tipo de Beneficio\" es obligatorio";
        die();
    }
    if ($beneficio_descrip == '') {
        echo "Error, el campo \"Descripción\" del apartado \"Beneficios\"es obligatorio";
        die();
    }
    ...
    if ($row = pg_query($conn, $SQL)) {
        echo "Se guardó con éxito el registro";
    } else {
...
if($accion == 4){
    ...
    if ($row = pg_query($conn, $SQL)) {
        echo "Se Modificó con éxito el registro";
    } else {
        echo "Ocurrió un error inesperado, por favor intentar más tarde";
    }
}else
