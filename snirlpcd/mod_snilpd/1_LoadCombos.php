...
/*	if ($_POST['cbNacionalidad_afiliado']=='2'){ COMENTADO 09102024 POR ERROR DE SUSTITUCION DE BD SERVIDORES
	    $param= " and id !='239'";
		}*/	
...
function LoadEstado_afiliado($conn){  
	...
	if ($GLOBALS[$sHtml_Var] == ''){
		$sSQL="SELECT id, nombre, status  FROM public.estado where pais_id='245' and status='A' ORDER BY nombre";		
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
...
function LoadAct_economica4($conn, $param){  
	...
	if ($GLOBALS[$sHtml_Var] == ''){	
	$sSQL="SELECT id, cod, nombre, cod_padre FROM public.actividad_eco where id >= '348' and status='A' ".$param."  ORDER BY nombre ASC";  
		$rs = &$conn->Execute($sSQL); 
		while ( !$rs->EOF ){
