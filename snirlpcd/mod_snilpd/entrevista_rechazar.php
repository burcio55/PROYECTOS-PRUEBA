...
if ($row0 = pg_query($conn, $entr)) {
    ...
    if ($row4 = pg_query($conn, $update)) {
        $rechazo = "UPDATE";
        $rechazo .= " snirlpcd.entrevista";
        $rechazo .= " SET";
        $rechazo .= " smotivo_rechazo = '$razon'";
        $rechazo .= " WHERE";
        $rechazo .= " id = '$entrevista_id'";
        if ($row5 = pg_query($conn, $rechazo)) {
            echo "Se rechazó con éxito la Entrevista";
        } else {
            echo "No se pudo rechazar la Entrevista";
        }
    } else {
        echo "No se pudo rechazar la Entrevista";
    }
