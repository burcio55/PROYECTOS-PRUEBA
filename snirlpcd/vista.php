...
$SQL="SELECT cedula
		FROM usuarios
		WHERE cedula = '".$_SESSION['nusuario']."' and clave='".$_SESSION["id_clave"]."'" ;

	$rs=$conn->Execute($SQL);


if( $rs->RecordCount()>0){
if(md5($_SESSION['usuario_id'])==$_SESSION["id_clave"]){	
//echo $rs->fields['resultado'];
//var_dump ($_SESSION);


	//$ced=$rs->fields['cedula'];
	//if(cedula_bd == cedula_sesion and cedula_bd==clave_sesion){}
	$_SESSION['vista'] = 1;
...
//debug();
	...
	//header('Location:../mod_registro/inicio.php');
	print "<script>document.location='mod_registro/inicio.php';</script>";
	}
}
