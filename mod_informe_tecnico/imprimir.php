...
/* $num_informe = $_SESSION['serial'];
$num_bpublico = $_SESSION['bien_publico'];
...
if ($num_informe == '') {
 ...
 $dispositivo = $_SESSION["id_dispositivo"];  */
...
inner join reporte_tecnico.estatus on estatus.id = reportes_fallas.estatus_id
where reportes_fallas.benabled='TRUE'  AND reportes_fallas.snro_reporte = '$id_imp' AND reportes_fallas.sserial = '$num_informe' AND  reportes_fallas.nbien_publico = '$num_bpublico' 
AND
reportes_fallas.snro_reporte is not null
AND
reportes_fallas.snro_reporte != 'No se ha generado un numero de reporte.'
...
while($persona = pg_fetch_assoc($row)){
    ...
    /* $pdf->$pdf->Image($imago, 15, 5, 300, 20); */
    ...
    /* $pdf->SetX(17);
    $pdf->SetFont('Arial', 'B', '10');
    $pdf->MultiCell(180, 8, utf8_decode('Recomendación(es): ').utf8_decode($recom), 1,'L');
    $pdf->Ln(3); */
$pdf->Ln(2);
$pdf->SetX(17);
