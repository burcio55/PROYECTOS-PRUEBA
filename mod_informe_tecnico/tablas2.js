function agregar_marca(){
    ...
    if (counter> "0"){
        ...
        $.ajax({
            url: 'subir2.php?accion=1&marca='+marc2,
            type: 'GET',
...
function borrar_marca(id){
    ...
    document.getElementById("borrar_marca").style.display = "none";
    ...
    // Mostrar la alerta
    document.getElementById("borrar_marca").style.display = "block";
    //CANCELAR
    document.querySelector("#borrar_marca button[data-bs-toggle='tooltip']").addEventListener("click", function() {
        document.getElementById("borrar_marca").style.display = "none";
    });

    // Agregar evento al botón "Continuar" (o cualquier otro botón)
    document.querySelector("#borrar_marca button[data-bs-toggle='tooltip2']").addEventListener("click", function() {
        document.getElementById("borrar_marca").style.display = "none";
        eliminar_marca(id)
    });
...
function editar_marca2(id_proceso, sdescripcion){   
    $.ajax({
        url: 'subir2.php?accion=3&id_proceso='+id_proceso+'&sdescripcion='+sdescripcion,
        method: 'GET',
        success: function(resp){
            let v0 = resp.split(" / ")[0];
            let v1 = resp.split(" / ")[1];
...
function editar_marca(id, sdescripcion){
...
function editar_marca(id_proceso, sdescripcion) {
