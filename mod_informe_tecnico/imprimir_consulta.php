...
$bien_publico = $_SESSION["bien_publico"];
...
where reportes_fallas.benabled='TRUE'  AND reportes_fallas.snro_reporte = '$id_imp' 
AND
...
inner Join public.ubicacion_administrativa on  ubicacion_administrativa.scodigo = reportes_fallas.ubicacion_administrativa_scodigo
inner join reporte_tecnico.estatus on estatus.id = reportes_fallas.estatus_id
where reportes_fallas.benabled='TRUE'  AND reportes_fallas.sserial = '$serial2' AND reportes_fallas.nbien_publico='$bien_publico2'
AND reportes_fallas.snro_reporte = '$id_imp' 
AND
reportes_fallas.snro_reporte is not null
AND
reportes_fallas.snro_reporte != 'No se ha generado un numero de reporte.'
...
if($nombreImagen == ''){
    $nombreImagen = 'No existe imagen en el informe';
}
...
WHERE reportes_fallas.snro_reporte = '$id_imp' LIMIT 1";
...
$pdf->Cell(0, 7, utf8_decode("N°: "). $id_imp."                   "."                   "."                   "."                   "."            "."  ".utf8_decode("Fecha de Impresión: ").$fechaActual, 0, 1, 'l');
...
$pdf->Cell(72, 8, $dispo, 1, 0, 'C');
$pdf->Cell(43, 8, $name, 1, 0, 'C');
...
$pdf->Cell(35, 8, utf8_decode('Bien Público'), 1, 0, 'C');
$pdf->Cell(40, 8, 'Estado Final', 1, 1, 'C');
...
$pdf->SetX(17);
$pdf->SetFont('Arial', 'B', '10');
$pdf->Cell(170, 8, 'Imagen Referencial: '.utf8_decode($nombreImagen), 0, 1, 'C');

/* if (empty($imagenPath)) {
    // Handle the case where image path is empty (e.g., display a placeholder)
    $pdf->Cell(300, 5, utf8_decode('Imagen no ingresada para el informe. Por favor hacerlo'), 0, 1, 'C'); 
} else { */
    if (file_exists($imagenPath) != FALSE) {
