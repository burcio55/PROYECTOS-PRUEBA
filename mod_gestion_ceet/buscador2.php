...
$PG2 .= " sprimer_nombre,";
$PG2 .= " ssegundo_nombre,";
$PG2 .= " aprimer_apellido,";
$PG2 .= " ssegundo_apellido,";
$PG2 .= " ssexo,";
$PG2 .= " stelefono_personal,";
$PG2 .= " entidad_nentidad,";
$PG2 .= " municipio_nmunicipio,";
$PG2 .= " parroquia_nparroquia,";
$PG2 .= " motor_id,";
$PG2 .= " actividad_economica";
$PG2 .= " FROM";
$PG2 .= " reporte_ceet.trabajador_indep";
$PG2 .= " WHERE";
$PG2 .= " ncedula='" . $_REQUEST['personales_cedula'] . "'";
$PG2 .= " AND";
$PG2 .= " snacionalidad='" . $_REQUEST['n_nacionalidad'] . "'";
$PG2 .= " AND";
$PG2 .= " sprimer_nombre='" . trim($valor['primer_nombre']) . "'";
$PG2 .= " AND";
$PG2 .= " ssegundo_nombre='" . trim($valor['segundo_nombre']) . "'";
$PG2 .= " AND";
$PG2 .= " aprimer_apellido='" . trim($valor['primer_apellido']) . "'";
$PG2 .= " AND";
$PG2 .= " ssegundo_apellido='" . trim($valor['segundo_apellido']) . "'";
$PG2 .= " AND";
$PG2 .= " ssexo='" . trim($valor['sexo']) . "'";
$PG2 .= " AND";
$PG2 .= " benabled='FALSE'";
$rs2 = pg_query($conn, $PG2); */
/* if (pg_num_rows($rs2) == 0) {
    echo " 7 / ";
} */

/* $telf = trim($valor2['stelefono_personal']);
$estado = trim($valor2['entidad_nentidad']);
$municipio = trim($valor2['municipio_nmunicipio']);
$parroquia = trim($valor2['parroquia_nparroquia']);
$motor = trim($valor2['motor_id']);
$act_economica = trim($valor2['actividad_economica']); */

/* $valores2 = pg_fetch_all($rs2); */

/* foreach ($valores2 as $valor2) {
    $personas .= trim($valor2['stelefono_personal']);
    $personas .= " / ";
    $personas .= trim($valor2['entidad_nentidad']);
    $personas .= " / ";
    $personas .= trim($valor2['municipio_nmunicipio']);
    $personas .= " / ";
    $personas .= trim($valor2['parroquia_nparroquia']);
    $personas .= " / ";
    $personas .= trim($valor2['motor_id']);
    $personas .= " / ";
    $personas .= trim($valor2['actividad_economica']);
    $personas .= " / ";
} */
