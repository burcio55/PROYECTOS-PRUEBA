...
function procesaroption(cedula, rolopcion){
	...
	var marcado = $('#user'+cedula+':checked').val()?true:false;
		if (!marcado) { //alert('No checked');
			$.ajax({
				url:'ajax_admin_usuario.php?cedula='+cedula+'&rolopcion='+rolopcion+'&rol='+rol+'&proceso=2',
			...
			$.ajax({
				url:'ajax_admin_usuario.php?cedula='+cedula+'&sistema='+sistema+'&rol='+rol+'&proceso=3',
				 beforeSend: function(objeto){
			  },
				success:function(data){
					//load();			
