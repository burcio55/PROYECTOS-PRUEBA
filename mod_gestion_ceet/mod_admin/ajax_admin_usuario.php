...
if(isset($_GET['proceso'])){
	...
	if($_GET['proceso']==3){
		...
		if ($rs4->RecordCount()>0){ // Trae todos los roles del sistema 
			...
			/*echo "<br> 2 ".*/ $SQL5="SELECT * FROM personales_rol WHERE nenabled=1 AND personales_cedula = '".$cedula."' AND rol_id IN (".$gurpo_id.");";	
			...
			if ($rs5->RecordCount()>0){ //echo $cedula." ENCONTRO ROL ACTIVO <br>";// Si encuentra un registro le hace nenable=0 y a su vez busca si el rol a crear esta agregado con valor  nenable=0
				...
				if ($rs7->RecordCount()>0){ //echo $cedula." ENCONTRO REGISTROS INABILITADO <br>";// Si encuentr el registro del rol con nenable=0 actualiza a nenable=1
					...
					/*echo "<br> 5 ".*/ $SQL8="UPDATE personales_rol SET nenabled=1, dfecha_actualizacion='".date('Y-m-d H:i:s')."', nusuario_actualizacion='".$_SESSION['id_usuario']."' WHERE id= '".$rolopcion."';";	
					...
					/*echo "<br> 6 ".*/ $SQL9="INSERT INTO personales_rol(personales_cedula, rol_id, nenabled, dfecha_creacion, nusuario_creacion) VALUES ('".$cedula."', '".$id_rol."', 1, '".date('Y-m-d H:i:s')."', '".$_SESSION['id_usuario']."');";	
				...
				/*echo "<br> 7 ".*/ $SQL10="INSERT INTO personales_rol(personales_cedula, rol_id, nenabled, dfecha_creacion, nusuario_creacion) VALUES ('".$cedula."', '".$id_rol."', 1, '".date('Y-m-d H:i:s')."', '".$_SESSION['id_usuario']."');";	
		...
		if ($rs5->RecordCount()>0){
			...
			while(!$rs5->EOF){
				...
				$cedula = trim($rs5->fields['cedula']);
					...
					$SQL6="SELECT * FROM personales_rol WHERE nenabled = 1 AND personales_cedula = '".$cedula."' AND rol_id IN (".$gurpo_id.");";	
					$rs6 = $conn->Execute($SQL6);
					...
					if ($rs6->RecordCount()>0){ //echo $cedula." ENCONTRO ROL ACTIVO <br>";// Si encuentra roles activos no hace nada...!
						...
						if ($rs8->RecordCount()>0){ //echo $cedula." ENCONTRO REGISTROS INABILITADO <br>";// Si encuentr el registro del rol con nenable=0 actualiza a nenable=1
							...
							$SQL9="UPDATE personales_rol SET nenabled=1, dfecha_actualizacion='".date('Y-m-d H:i:s')."', nusuario_actualizacion='".$_SESSION['id_usuario']."' WHERE id= '".$rolopcion."';";	
							$rs9 = $conn->Execute($SQL9);
							...
							$SQL10="INSERT INTO personales_rol(personales_cedula, rol_id, nenabled, dfecha_creacion, nusuario_creacion) VALUES ('".$cedula."', '".$rol."', 1, '".date('Y-m-d H:i:s')."', '".$_SESSION['id_usuario']."');";	
							$rs10 = $conn->Execute($SQL10);
	...
	if($_GET['proceso']==5){
		...
		$rol = $_GET['rol'];
			...
			$SQL11="UPDATE personales_rol SET nenabled=0, dfecha_actualizacion='".date('Y-m-d H:i:s')."', nusuario_actualizacion='".$_SESSION['id_usuario']."' WHERE rol_id= '".$rol."';";	
