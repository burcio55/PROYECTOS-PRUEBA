...
if ($accion == 1) {
    ...
    if ($cuenta_contable == "") {
        ...
        $sql2 = "SELECT * FROM bienes_publicos.cuenta_contable WHERE sdescripcion='$cuenta_contable'  AND benabled = 'TRUE'";
        $rs2 = pg_query($conn, $sql2);
        if (pg_num_rows($rs2) > 0) {
            echo "0 / Este Bien Público ya está registrado";
        } else {

            $sql = "INSERT INTO bienes_publicos.cuenta_contable(sdescripcion,  nusuario_creacion) VALUES ('" . $cuenta_contable . "', '" . $id_usuario . "')";
            if ($row = pg_query($conn, $sql)) {

                $i = 0;
                $sql2 = "SELECT id, sdescripcion FROM bienes_publicos.cuenta_contable WHERE benabled='TRUE' ORDER BY sdescripcion ASC";
                ...
                while ($or = pg_fetch_assoc($row2)) {
                    ...
                    $cosa .= "<tr>
                            ...
                            <td class=\"td center\" id=\"botones\">
                                <button type=\"button\" class=\"btn btn-warning\" style=\"background-color: #46A2FD; color: #fff; border: 1px Solid #46A2FD; padding: 7px 22px; border-radius: 30px; width: auto;                  \" onclick=\"accion_modificar6('" . $or['id'] . "','" . $or['sdescripcion'] . "')\">Modificar</button>";
...
if ($accion == 2) {
    ...
    $a = "SELECT * FROM bienes_publicos.bienes_publicos WHERE benabled = 'TRUE' AND cuenta_contable_id='" . $id . "'";
    ...
    if (pg_num_rows($res) > 0) {
        ...
        /* echo "1 / ". $id; */
        $i = 0;
        $SQL2 = "UPDATE bienes_publicos.cuenta_contable SET benabled = 'FALSE' WHERE id = '" . $id . "'";
        if ($resultado2 = pg_query($conn, $SQL2)) {
            $i2 = 0;
            $sql2 = "SELECT * FROM bienes_publicos.cuenta_contable WHERE benabled = 'TRUE' Order By sdescripcion";
            $row2 = pg_query($conn, $sql2);
            ...
            while ($or2 = pg_fetch_assoc($row2)) {
                ...
                $cosa2 .= "<tr>
                    ...
                    <td class=\"td center\" id=\"botones\">
                    <button type=\"button\" class=\"btn btn-warning\" style=\"background-color: #46A2FD; color: #fff; border: 1px Solid #46A2FD; padding: 7px 22px; border-radius: 30px; width: auto;                  \" onclick=\"accion_modificar6('" . $or2['id'] . "','" . $or2['sdescripcion'] . "')\">Modificar</button>";
                if ($_SESSION['bienes_publicos_rol'] == 80) {
...
if ($accion == 3) {
    $id3 = $_REQUEST["id_mantenimiento7"];
    $cuenta_contable3 = $_REQUEST["cc2"];
    $id_usuario3 = $_SESSION["id_usuario"];
    /*  echo "1 / ".$id3." ".$origen3." ".$id_usuario3; */
    $SQL3 = "UPDATE bienes_publicos.cuenta_contable SET sdescripcion='" . $cuenta_contable3 . "', nusuario_actualizacion='" . $id_usuario3 . "' WHERE id ='" . $id3 . "'";
    if ($resultado3 = pg_query($conn, $SQL3)) {
        $i3 = 0;
        $sql3 = "SELECT * FROM bienes_publicos.cuenta_contable WHERE benabled = 'TRUE' Order By sdescripcion";
        ...
        while ($or3 = pg_fetch_assoc($row3)) {
            ...
            <td class=\"td center\" id=\"botones\">
            <button type=\"button\" class=\"btn btn-warning\" style=\"background-color: #46A2FD; color: #fff; border: 1px Solid #46A2FD; padding: 7px 22px; border-radius: 30px; width: auto;\" onclick=\"accion_modificar6('" . $or3['id'] . "','" . $or3['sdescripcion'] . "')\">Modificar</button>";
            if ($_SESSION['bienes_publicos_rol'] == 80) {
