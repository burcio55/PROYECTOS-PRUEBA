...
function doAction($conn){
	if (isset($_POST['action'])){
				...
				case 'consultar':
						...
						if ($_POST['txt_tipo_trabajador']!='OBRERO') $query = ($_POST['quincena']) ? $_POST['quincena'] : $_POST['nronomina_cb'];
...
<?php include('funciones_generales.php'); ?>
    ...
    <input name="txt_fecha_ingreso" id="txt_fecha_ingreso" type="hidden" value="<?= $aDefaultForm['txt_fecha_ingreso']; ?>" />
    ...
    <input name="txt_ubicacion_fisica" id="txt_ubicacion_fisica" type="hidden" value="<?= $aDefaultForm['txt_ubicacion_fisica']; ?>" />
		...
		<table width="95%" border="0" align="center" class="formulario">
			<tr>
				<th class="titulo" colspan="4" align="center">CONSULTA DE RECIBO DE PAGO QUINCENAL DEL AÑO ACTUAL</th>
			</tr>
			...
			<tr>
				<td></td>
				<th colspan="2" class="sub_titulo_3"><div align="center">PERIODO A CONSULTAR</div></th>
				<td></td>
			</tr>
			...
			<tr align=left >
				<td></td>
				<th width="25%" class="sub_titulo"><div align="center">Mes</div></th>
				<th width="25%" class="sub_titulo"><div align="center"><?php if($aDefaultForm['txt_tipo_trabajador']!='OBRERO'){echo 'Quincena';}else{echo 'Semana';}?></div></th>
			</tr>
			...
			<tr align=left>
				...
				<td class="dataListColumn3">
					<div align="center"><font color="#666666">
						<select name="mes_cb" class="textbox" id="mes_cb" onchange="send('recarga')">
						<option value="0" selected="selected">Seleccione</option>
						<? loadMes_cb() ; print $GLOBALS['sHtml_mes_cb']; ?>
						</select></font>
					</div>
				</td>
				<? if ($aDefaultForm['txt_tipo_trabajador']!='OBRERO'){
					if (!preg_match('/(10|11)/', $aDefaultForm['mes_cb'])){
          ...
          <? } 
			...
			<tr>
				<td colspan="5" align="center"><button type="button" class="button_personal btn_buscar" onclick="javascript:send('consultar');" title="Haga Click para buscar">Buscar</button> </td>
			</tr>
...
if($_POST['action']=='consultar'){
	...
	if (!preg_match('/(10|11)/', $aDefaultForm['mes_cb']))
		 $nro_semana="recibo_pago.nsemana_quincena=".$nros[$_POST['quincena_cb']-1];
		else
		 $nro_semana="recibo_pago.nnro_nomina=".$_POST['nronomina_cb'];
