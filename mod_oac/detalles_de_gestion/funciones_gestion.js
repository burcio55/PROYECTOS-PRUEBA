function mostrar_formulario(){
	$.ajax({
	type: 'POST',
	url: 'formulario_gestion.php',
	success: function(data) {
		$('#formulario_gestion').html(data);
		$('#btnAgregar').show();
		$('#btnModificar').hide();
	 }
  });	
	...
	if(validar_campos()==true){
		$.ajax({
			...
			url: '../case_sentencias.php',
	  	    data:'txt_descripcion='+document.getElementById("txt_descripcion").value+'&opcion='+12,
			success: function(data) {
				if(data=='guardado'){		
...
function modificar(){
		...
		$.ajax({
			...
			'&opcion='+13+'&id='+document.getElementById("id").value,
			success: function(data) {
					//alert(data);
					if(data=='modificado'){		
