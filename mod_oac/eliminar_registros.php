...
if (isset($_POST['action'])){
		...
		switch($_POST['action']){
			...
			?><script>				
				document.location='cambiar_estatus_caso.php';
			</script><?
				LoadData($conn,false);		
...
function ProcessForm($conn){
	...
	if($_POST['observaciones']=='')$_POST['observaciones']='SIN OBSERVACIONES';
  ...
  dfecha_actualizacion= '".$fecha_hoy."',
		...
		WHERE id_detalle_atencion=".$_POST['id_caso']." and id_datos_personales=".$_SESSION['id_datos_personales']."";
		$rs5=$conn->Execute($sq);
		 if($rs5){
			echo "<script>alert('El caso ".$_SESSION['nun_caso']." ha sido eliminado');
...
<!--	<tr>
		...
		<th class="sub_titulo" width="169"><div align="center">Fecha Recepci&oacute;n</div></th>
	...
	<tr>
		...
		<td align="center"><font color="#666666"><select style="border-radius: 30px; border-color:#999999; width:75%" id="cbo_recepcion"  disabled="disabled" sname="cbo_recepcion">
...
<table width="95%" align="center" class="formulario" border="0">
    ...
    <tr>
     ...
     <tr>
       <td colspan="2"><font color="#666666" >
            <select style="border-radius: 30px; border-color:#999999; width:95%" id="cbo_asistencia" name="cbo_asistencia" disabled="disabled"  title="Tipo de Asistencia - Seleccione solo una opcion del listado" onChange="JavaScript:send('Actualizar_combos')">
       ...
       <td colspan="3"><font color="#666666" >
            <select style="border-radius: 30px; border-color:#999999; width:95%" id="cbo_detalle_gestion" name="cbo_detalle_gestion"disabled="disabled" "title="Detalle de Gesti&oacute;n - Seleccione solo una opcion del listado"onChange="JavaScript:send('Actualizar_combos')">
            <option value="0">Seleccione</option>
     ...
     <? if($aDefaultForm['cbo_detalle_gestion']=='2' && $aDefaultForm['cbo_asistencia']=='1') {?>
     <td>
            <select id="cbo_tipo_caso_rnet" name="cbo_tipo_caso_rnet" disabled="disabled" style="width:95%"title="Tipo de Caso RNET - Seleccione solo una opcion del listado" onChange="JavaScript:send('Actualizar_combos')">
            <option value="0">Seleccione</option>
	 ...
	 if($aDefaultForm['cbo_asistencia']=='1' && $aDefaultForm['cbo_tipo_caso_rnet']=='1' && $_POST['cbo_detalle_gestion']=='2'){ ?>
     <td>
           <select id="cbo_detalle_caso_rnet" name="cbo_detalle_caso_rnet" disabled="disabled"style="width:95%"title="Detalle del Caso RNET - Seleccione solo una opcion del listado"onChange="JavaScript:send('Actualizar_combos')">
            <option value="0">Seleccione</option>
     ...
     <td colspan="3">
     		<select id="cbo_dato_corregir_rnet" name="cbo_dato_corregir_rnet" disabled="disabled" style="width:95%"title="Dato a Corregir - Seleccione solo una opcion del listado"onChange="JavaScript:send('Actualizar_combos')">
             <option value="0">Seleccione</option>
     ...
     <tr>
       <th colspan="5" class="sub_titulo" >Planteamiento del Caso</th>
     </tr>
	 
     <tr>
     <td colspan="5" ><font color="#666666"><textarea id="planteamiento_caso" name="planteamiento_caso"  disabled="disabled"cols="125%" rows="2%"  style="width:95%"title="Planteamiento del Caso - Ingrese letras y/o numeros" onkeyup="mayusculas(this);" class="areatexto" placeholder="Planteamiento del Caso"><? print $aDefaultForm['planteamiento_caso'];?></textarea>
       <span class="requerido"></span></font></td>
     </tr>
    ...
    <tr>
       <td colspan="2"><font color="#666666">
       <input name="srif" type="text"  id="srif"   onkeyup="mayusculas(this);" disabled="disabled"placeholder="R.I.F" value="<?=$aDefaultForm['srif'];?>" >
       </strong> 
       <button type="button" class="button_personal btn_buscar" disabled="disabled" onClick="JavaScript:send('cmd_buscar_empresa')">Buscar</button> <span class="requerido"> Ej. J123456789</span></font></td>
    ...
    <tr> 
      <td colspan="5">
            ...
            <select name="cbo_sector" disabled="disabled" class=""  <? //print $bloqueado_boton ?>style="width:95%" title="Sector - Seleccione una opción del listado" id="cbo_sector">
...
<tr>
    ...
    <tr>
      <td colspan="5" >
        <input name="email_contacto1" type="text" id="email_contacto1"  disabled="disabled"  value="<?=$aDefaultForm['email_contacto1'];?>" size="50" placeholder="micorreo" title="Correo electronico -  Ejemplo: micorreo,jperez,inv123.ca,otros" /> @ <input name="email_contacto2" type="text" id="email_contacto2" onblur="validarEmail()"  disabled="disabled" value="<?=$aDefaultForm['email_contacto2'];?>" size="50" placeholder="gmail.com" title="Correo electronico -  Ejemplo: gmail.com,hotmail.com,otros"/></td>      
    </tr>
<? }?>
    ...
    <tr>
      <td colspan="2"><font color="#666666">
        <input style="border-radius: 30px; border-color:#999999; width:85%" name="persona_remite_caso" type="text"  id="persona_remite_caso"  size="70" placeholder="Nombre(s) y Apellido(s)" onkeyup="mayusculas(this);" value="<?=$aDefaultForm['persona_remite_caso'];?>" disabled></font></td>
    ...
    <tr>
      <td colspan="2" align="left"><font color="#666666">
        <input style="border-radius: 30px; border-color:#999999; width:85%" name="f_remision" id="f_remision" type="text"  title="Fecha de Remisi&oacute; - Seleccione en el Calendario" size="10"  value="<?= $aDefaultForm['f_remision'];?>" readonly disabled/>
      ...
      <td colspan="3"><font color="#666666">
      <input style="border-radius: 30px; border-color:#999999; width:85%" name="numero_memo" type="text" id="numero_memo" placeholder="Ej.123-A" value="<?=$aDefaultForm['numero_memo'];?>" disabled>
    </tr>
 ...
 <!--    <tr>
      <td colspan="2"><font color="#666666">
          <select style="border-radius: 30px; border-color:#999999; width:85%" id="cbo_resultado" name="cbo_resultado" disabled="disabled" title="Resultado - Seleccione una opción del listado" >
          <option value="">Seleccione</option>
	...
	<tr>	
     ...
     <button type="button" id="enviar" name="enviar" class="button_personal btn_guardar" onClick="send('guardar')"  >Eliminar</button>
