function mostrar_formulario(){
	$.ajax({
	type: 'POST',
	url: 'formulario_dato_rnet.php',
	success: function(data) {
		$('#formulario_dato_rnet').html(data);
		$('#btnAgregar').show();
		$('#btnModificar').hide();
	 }
  });	
	...
	if(validar_campos()==true){
		$.ajax({
			...
			url: '../case_sentencias.php',
	  	    data:'txt_descripcion='+document.getElementById("txt_descripcion").value+'&opcion='+18,
			success: function(data) {
				if(data=='guardado'){		
...
function modificar(){
		...
		$.ajax({
			...
			'&opcion='+19+'&id='+document.getElementById("id").value,
			success: function(data) {
					//alert(data);
					if(data=='modificado'){		
