 function mostrar_formulario(){
	$.ajax({
	type: 'POST',
	url: 'formulario_estatus_caso.php',
	success: function(data) {
		$('#formulario_estatus_caso').html(data);
		$('#btnAgregar').show();
		$('#btnModificar').hide();
	 }
  });	
...
function registro_tabla(){
	...
	success: function(data) {
			var str=data;
			var n=str.split("|");
			$('#tabla_intro_estatus_caso').html(n[0]); 
			if(n[1]==0){
				$('#cant_campos').val('0');	
			}else{
				$('#cant_campos').val(n[1]);	
	...
	if(validar_campos()==true){
		$.ajax({
			...
			url: '../case_sentencias.php',
	  	    data:'txt_descripcion='+document.getElementById("txt_descripcion").value+'&opcion='+8,
			success: function(data) {
				if(data=='guardado'){		
...
function modificar(){
		...
		$.ajax({
			...
			'&opcion='+2+'&id='+document.getElementById("id").value,
			success: function(data) {
					//alert(data);
					if(data=='modificado'){		
