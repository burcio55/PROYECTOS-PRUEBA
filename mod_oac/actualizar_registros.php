...
if (isset($_POST['action'])){
		...
		//	echo"los datos1";
			...
			?><script>
				if (confirm("¿Está seguro que quiere Actualizar el Registro?")) {
    			document.location = 'actualizar_registros2.php?pas=<?= $pase ?>';
			} 
			</script><?
			...
			?><script>
    			...
    			document.location = 'eliminar_registros.php?pas=<?= $pase ?>';
			} 
			</script><?
...
?>	<div id="Contenido" text-align="center" style="overflow:auto">
	<br>
...
<tr>
<th colspan="4"  class="sub_titulo"><div align="left">ACTUALIZACI&Oacute;N --> Actualizar Caso </div></th>
</tr>
<tr>
...
<tr class="identificacion_seccion">
        <th style="border-radius: 30px; border-color:#999999; width:85%" colspan="6" class="sub_titulo_2" align="left">DATOS DEL CIUDADANO </th>
 </tr>
   ...
   <tr>
   	...
   	<tr class="identificacion_seccion">
        <th style="border-radius: 30px; border-color:#999999; width:85%" colspan="6" class="sub_titulo" align="left">Datos B&aacute;sicos</th>
	</tr>
	...
	<tr>  
		<th style="color:#666"><div text-align="left">Nombre(s) y Apellido(s)</div> </th>
		<th style="color:#666"><div text-align="left">Edad</div></th>
		<th style="color:#666"><div text-align="left">Sexo</div></th> 
		<th style="color:#666"><div text-align="left">Beneficiario</div></th>  
		<th style="color:#666"><div text-align="left">N&uacute;mero de Teléfono </div></th>
 	    <th style="color:#666"><div text-align="left">Correo Electr&oacute;nico</div></th>
	</tr>

	<tr text-align="center">  
	   <td text-align="left"><font color="#666666"> <? print $aDefaultForm['apellidonombre'];?> </font></td>
	   <td text-align="left"><font color="#666666"><? print $aDefaultForm['edad'];?>  a&ntilde;os </font></td>
	   <td text-align="left"><font color="#666666"> <? if ($aDefaultForm['sexo']=='1') print "Masculino";
			if ($aDefaultForm['sexo']=='2') print "Femenino";?>   </font></td>  
	   <td text-align="left"><font color="#666666">
...
// Verificar si la sesión tiene un valor válido
if (!isset($_SESSION['id_usuario'])) {
    die("Usuario no autenticado.");
}

$locust = "SELECT * FROM public.personales_rol WHERE personales_cedula = '".$_SESSION['id_usuario']."' AND rol_id = '58'";
$rolex = $conn->Execute($locust);
...
$userHasRole = false;
$id_rol = null; // Inicializar $id_rol

if ($rolex) {
    while ($fila = $rolex->FetchRow()) {
        if ($fila['rol_id'] == '58') {
            $userHasRole = true;
            $id_rol = $fila['rol_id'];
            break; // Salir del bucle una vez encontrado el rol
        }
    }
}

/*  echo "User has role: " . ($userHasRole ? "true" : "false") . " " . ($id_rol ? $id_rol : "No role found");  */
?>

<table class="display formulario" border="0" align="center" id="tblDetalle" width="98%">
    <thead>
        <tr>
            <th style="border-radius: 30px; border-color:#999999; width: 100%" colspan="9" class="sub_titulo_2" height="20" align="left">SOLICITUDES REALIZADAS POR EL CIUDADANO</th>
        </tr>
        ...
        <tr>
            ...
            <th width="10%" align="left" class="sub_titulo"><div align="center">Fecha de Recepción</div></th>
            <th width="10%" align="left" class="sub_titulo"><div align="center">Beneficiario</div></th>
            <th width="10%" align="left" class="sub_titulo"><div align="center">Vía de Recepción</div></th>
            ...
            <th width="12%" align="left" class="sub_titulo"><div align="center">Estatus</div></th>
            <th width="12%" align="left" class="sub_titulo"><div align="center">Acciones</div></th>
        </tr>
    </thead>
    <tbody>
        <?php
        ...
        ?>
            <tr>
                ...
                <td class="texto-normal" align="center"><?= $aTabla[$c]['sdescripcion_status'] ?></td>
                <td class="texto-normal" align="center">
                    <div style="display: flex; gap: 10px; justify-content: center;">
                        <?php if ($aTabla[$c]['id_status'] == '1') { ?>
                            <button align="center" style="background-color:gray; border-radius: 50px; border: none; padding: 5px 10px" onclick="seleccionar('siguiente', <?= $id; ?>);" disabled>
                                Modificar
                            </button>
                            <button align="center" style="background-color:gray; border-radius: 50px; border: none; padding: 5px 10px" onclick="seleccionar('siguiente2', <?= $id; ?>);" disabled>
                                Eliminar
                            </button>
                        <?php } elseif ($aTabla[$c]['id_status'] == '2') { ?>
                            <button style="background-color:#D0E0F4;; border-radius: 50px; border: none; padding: 5px 10px;" onclick="seleccionar('siguiente', <?= $id; ?>);">
                                Modificar
                            </button>
                            <?php if ($userHasRole) { ?>
                                <button id="miBoton" style="background-color:indianred; border-radius: 50px; border: none; padding: 5px 10px;" onclick="seleccionar('siguiente2', <?= $id; ?>);">
                                    Eliminar
                                </button>
                            <?php } else { ?>
                                <!-- <p>No tienes permiso para ver este botón.</p> -->
                            <?php } ?>
                        <?php } ?>
                    </div>
                </td>
