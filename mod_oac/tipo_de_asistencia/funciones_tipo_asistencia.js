function mostrar_formulario(){
	$.ajax({
	type: 'POST',
	url: 'formulario_tipo_asistencia.php',
	success: function(data) {
		$('#formulario_tipo_asistencia').html(data);
		$('#btnAgregar').show();
		$('#btnModificar').hide();
	 }
  });	
	...
	if(validar_campos()==true){
		$.ajax({
			...
			url: '../case_sentencias.php',
	  	    data:'txt_descripcion='+document.getElementById("txt_descripcion").value+'&opcion='+3,
			success: function(data) {
				if(data=='guardado'){		
...
function editar(id){
$('#formulario_tipo_asistencia').show();
...
function modificar(){
		...
		$.ajax({
			...
			'&opcion='+4+'&id='+document.getElementById("id").value,
			success: function(data) {
					//alert(data);
					if(data=='modificado'){		
